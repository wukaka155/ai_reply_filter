...
**🎉 v1.5.0 新增**: 消息合并功能优化，群聊消息智能标注发送者！
...
- 🆕 **消息合并等待**: 收集群内连续消息合并处理，**自动标注每个用户**，防止混淆
...
群聊场景下，收集连续消息合并处理，**自动标注每个用户**，避免混淆：
...
```
用户A [t=0s]: "我想问一下"
用户B [t=2s]: "关于Python的问题"
用户A [t=4s]: "如何使用asyncio?"

→ AI在t=10s收到合并消息:
"[用户A昵称(123)] 我想问一下
[用户B昵称(456)] 关于Python的问题
[用户A昵称(123)] 如何使用asyncio?"

→ 完整的群聊上下文 + 清晰的发送者标注 = 更准确的回复
```

**特点**：
- ✅ 自动标注每条消息的**发送者昵称和ID**
- ✅ 收集所有用户的消息，提供**完整群聊上下文**
- ✅ 防止昵称伪造等安全问题
- ✅ 自动等待后续消息，达到最大数量立即处理
- ✅ 避免频繁打断用户输入
...
A: **不会混淆！** v1.5.0优化了这个机制：
- 群聊中所有用户的消息会一起合并（提供完整对话上下文）
- 但每条消息都会标注 `[发送者昵称(用户ID)] 消息内容`
- AI能清楚知道每条消息是谁发的，不会混淆
- 有效防止昵称伪造等安全隐患
...
### v1.5.0 (2025-11-22)
- 🎉 **重要优化**: 消息合并功能安全性和上下文改进
- 🔒 **安全修复**: 为每条合并消息标注发送者昵称和ID，防止混淆
- ✨ 改进合并策略：群聊中所有用户消息一起合并（完整对话上下文）
- ✨ 消息格式优化：`[发送者昵称(ID)] 消息内容`
- 🛡️ 有效防止昵称伪造等安全隐患
- 📝 优化日志输出，清晰显示合并过程和用户信息
- 🎯 更符合群聊实际场景，AI能理解完整的多人对话
