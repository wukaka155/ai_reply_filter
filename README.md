...
- ✅ **上下文感知**: 可结合历史消息上下文进行判断
- ✅ **人设适配**: 支持根据AI人设判断消息是否符合回复范围
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| AI助手人设上下文 | 字符串 | "" | 描述AI助手的人设、性格、角色定位等信息 |
| AI判断系统提示词 | 字符串 | (默认提示词) | 指导AI如何判断消息是否需要回复的系统提示词 |
| 上下文消息数量 | 整数 | 0 | 判断时包含的历史消息数量，0=不使用上下文 |
| AI判断超时时间 | 整数 | 10 | 调用AI分析的最长等待时间（秒） |

### 高级配置

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| 阻止模式 | 整数 | 1 | 0=完全阻止且不保存记录，1=阻止AI响应但保存记录 |
| 启用结果缓存 | 布尔值 | true | 是否缓存相同消息的AI判断结果 |
| 缓存过期时间 | 整数 | 300 | AI判断结果的缓存有效期（秒） |
...
让AI根据设定的人设判断消息是否符合回复范围：

**配置示例**：
```
AI助手人设上下文: "你是一个专业的Python编程助手，只回答与Python编程相关的问题。"
```

这样AI会自动过滤掉非编程相关的闲聊消息。
